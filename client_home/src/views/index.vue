		    ...
		    <div class="container">
		        <div class="row">
		            <div class="col">
						...
						<!-- 推荐招标项目模块(开始) -->
		<div class="warp" v-if="$check_action('/bidding_project/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/bidding_project/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_bidding_project = !show_list_bidding_project">
		                        <div class="switch_box">
		                            <span v-if="show_list_bidding_project"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 招标项目推荐列表组件 -->
		                <list_bidding_project v-if="show_list_bidding_project" :list="list_bidding_project" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_bidding_project.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_bidding_project" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_bidding_project" :key="i" @click="to_details('bidding_project',o,'bidding_project_id')">
		                            <td v-for="(oj,n) in fields_bidding_project">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				show_list_article: true,
								show_list_bidding_project: true,
				list_bidding_project: [],
				vm_arr_bidding_project: [
					"bidding_project_id",
							"bidding_project",
		...
		methods: {
			...
			toToggle(){
				this.isAdmin = !this.isAdmin;
							...
							// 获取招标项目列表
			get_bidding_project() {
				let url = "~/api/bidding_project/get_list?";
				...
				url = url_get_list;
					...
					if (json.result) {
						this.list_bidding_project = json.result.list;
					}
				})
			},
		...
		computed: {
						    list_table_help_center() {
		        var list = this.list_bidding_project;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_bidding_project.map((o) => {
		                // 第二个中括号是对象的属性
