		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="责任单位" prop="responsible_unit">
												<el-input id="responsible_unit" v-model="form_sub['responsible_unit']" placeholder="请输入责任单位"
							  v-if="user_group === '管理员' || (form_sub['responsible_unit_id'] && $check_field('set','responsible_unit')) || (!form_sub['responsible_unit_id'] && $check_field('add','responsible_unit'))"></el-input>
					<div v-else-if="$check_field('get','responsible_unit')">{{form_sub['responsible_unit']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="单位地址" prop="work_address">
												<el-input id="work_address" v-model="form_sub['work_address']" placeholder="请输入单位地址"
							  v-if="user_group === '管理员' || (form_sub['responsible_unit_id'] && $check_field('set','work_address')) || (!form_sub['responsible_unit_id'] && $check_field('add','work_address'))"></el-input>
					<div v-else-if="$check_field('get','work_address')">{{form_sub['work_address']}}</div>
											</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["responsible_unit_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?responsible_unit_id=" + form_sub["responsible_unit_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/responsible_unit/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/responsible_unit/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/responsible_unit/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/responsible_unit/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/responsible_unit/view','get');
